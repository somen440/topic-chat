...
const TOPIC_CATALOG_SERVICE_ADDR =
  process.env.TOPIC_CATALOG_SERVICE_ADDR ?? "localhost:9092";
...
//
const state = {
  client: new TopicCatalogServiceClient(
    `${SCHEME}://${TOPIC_CATALOG_SERVICE_ADDR}`
  )
};
...
const actions = {
  ...
  getAll(
    context: TopicCatalogContext
  ): Promise<ClientReadableStream<ListTopicsResponse>> {
    const req = new Empty();
    console.log("req");
